...
public class SocketChannelIOHelper {
	...
	public static boolean batch(WebSocketImpl ws, ByteChannel sockchannel) throws IOException {
		...
		if (ws.outQueue.isEmpty() && ws
				.isFlushAndClose() /* && ( c == null || c.isNeedWrite() ) */) {
			synchronized (ws) {
				ws.closeConnection();
			}
		}
	...
	public static void writeBlocking(WebSocketImpl ws, ByteChannel channel) throws InterruptedException, IOException {
		assert ((channel instanceof AbstractSelectableChannel) == true
				? ((AbstractSelectableChannel) channel).isBlocking() : true);
