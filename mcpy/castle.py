    ...
    for x in range(min(x1,x2),max(x1,x2)+1):
        ...
        for y in range(min(y1,y2),max(y1,y2)+1):
            ...
            for z in range(min(z1,z2),max(z1,z2)+1):
                ...
                if isinstance(distribution, block.Block):
...
distribution = ((.05,block.MOSS_STONE), (.1,block.Block(block.STONE_BRICK.id, 1)), 

                (.2,block.Block(block.STONE_BRICK.id, 2)),

                (.651,block.Block(block.STONE_BRICK.id, 0)))
